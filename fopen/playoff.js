...
function init() {
  ...
  presetEl.addEventListener('change', ()=> { state.preset=presetEl.value; state.matches={}; buildBracket(); saveState(); });
...
function onBackupFile(e){
  ...
  reader.onload=()=>{
    ...
    if (backupData?.playoff?.matches) {
      state.preset = backupData.playoff.preset || state.preset;
      state.matches = backupData.playoff.matches || {};
      document.getElementById('preset').value = state.preset;
    }
    teams = extractTeams(backupData.sourceBackup || backupData);
...
function extractTeams(data){
  ...
  if (Array.isArray(data?.sourceBackup?.selectedTeams)) return data.sourceBackup.selectedTeams;
  return [];
}
...
function buildBracket(){
  ...
  preset.rounds.forEach(r=>{
    const sec=document.createElement('div'); sec.className='playoff-round';
...
function renderMatch(id){
  const m=ensureMatch(id); const el=document.createElement('div'); el.className='playoff-match';
...
function deriveWinner(match){
  ...
  if(Number.isNaN(sh) || Number.isNaN(sa) || sh===sa) return '';
  return sh>sa ? match.home : match.away;
}

function recomputeAllWinners(){
  Object.values(state.matches||{}).forEach(m=>{ if(m) m.winner = m.winner || deriveWinner(m); });
}
...
function exportFullTournament(){
  ...
  const fw = deriveWinner(state.matches.F || {});
  const bw = deriveWinner(state.matches.BR || {});
  if(fw && bw){
    const f=state.matches.F;
    out.finalStandings={ gold:fw, silver:fw===f.home?f.away:f.home, bronze:bw };
  }
