(() => {
  ...
  function replaceRequired(source, search, replacement, label) {
    ...
    if (next === source) console.warn(`TURN baseline stability not applied: ${label}`);
    return next;
  }
...
renderer.setAnimationLoop((now) => {
  ...
  window.fetch = async (input, init) => {
    ...
    try {
      ...
      console.warn('TURN baseline stability layer failed, using upstream game source.', error);
      return response;
    }
  };
})();