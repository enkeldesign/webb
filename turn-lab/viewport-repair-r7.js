(() => {
  ...
  const AUTO_RETRY_DELAYS_MS = Object.freeze([1500, 5000, 15000]);
  ...
  let runtimeReadySeen = Boolean(globalThis.__turnRuntime && globalThis.__turnNextRaceSession);
  let homeReadySeen = Boolean(document.querySelector('.m8-home'));
  const autoHistory = [];
  ...
  function loadingVisible() {
    const loading = document.querySelector('.turn-loading-screen, #turnLoadingScreen, [data-turn-loading]');
    if (!loading) return false;
    const style = getComputedStyle(loading);
    const rect = loading.getBoundingClientRect();
    return !loading.hidden && style.display !== 'none' && style.visibility !== 'hidden' && rect.width > 0 && rect.height > 0;
  }
  ...
  function appState() {
    return [
      ...
      runtimeReadySeen,
      homeReadySeen || Boolean(document.querySelector('.m8-home')),
      loadingVisible()
    ];
  }
  ...
  function autoAllowed() {
    return document.visibilityState === 'visible' && homeReadySeen;
  }
  ...
  function benchUi() {
    ...
    return {
      ...
      gateText: bench?.querySelector('[data-lab-repair-gate]') || null,
  ...
  function syncBenchUi(sample = null) {
    ...
    if (ui.gateText) {
      ui.gateText.textContent = `AUTO GATE · ${homeReadySeen ? 'HOME READY' : 'WAITING FOR HOME'} · attempts ${autoAttemptCount}`;
    }
  ...
  function compactAutoAttempt(result) {
    const after = result.checkpoints?.at(-1) || null;
    return {
      attempt: autoAttemptCount,
  ...
  function scheduleAutoRetry() {
    clearRetryTimer();
    const delayMs = AUTO_RETRY_DELAYS_MS[autoAttemptCount - 1];
    if (!Number.isFinite(delayMs)) return;
    retryTimer = window.setTimeout(() => {
      retryTimer = 0;
      if (!autoAllowed() || repairInFlight) return;
      incidentArmed = true;
      confirmAutoRepair(`retry+${delayMs}`);
    }, delayMs);
  }
  ...
  async function runMetaReflow({ trigger = 'manual' } = {}) {
    ...
    if (ui.status) {
      ui.status.textContent = trigger === 'auto'
        ? `Known BAD viewport detected. Automatic repair attempt ${autoAttemptCount}…`
    ...
    try {
      ...
      const result = {
        lab: 'TURN viewport repair bench r7 staged lifecycle watchdog',
      ...
      if (trigger === 'auto') {
        ...
        if (autoHistory.length > 4) autoHistory.shift();
        globalThis.__turnLabViewportAutoRepairResult = result;
      ...
      if (currentUi.status) {
        if (recovered) {
          currentUi.status.textContent = `${trigger === 'auto' ? 'AUTO-RECOVERED' : 'RECOVERED'}: WebKit restored the full reachable viewport.`;
        } else if (trigger === 'auto' && autoAttemptCount <= AUTO_RETRY_DELAYS_MS.length) {
          const retryDelay = AUTO_RETRY_DELAYS_MS[autoAttemptCount - 1];
          currentUi.status.textContent = `AUTO REPAIR ATTEMPT ${autoAttemptCount} FAILED: retrying in ${Math.round(retryDelay / 100) / 10}s if still BAD.`;
        } else {
          currentUi.status.textContent = `${trigger === 'auto' ? 'AUTO REPAIR FAILED' : 'STILL BAD'}: viewport-meta reflow did not recover the missing height.`;
        }
      }
      window.dispatchEvent(new CustomEvent('turn-lab:viewport-repair-result', {
        detail: { trigger, outcome: result.outcome, attempt: autoAttemptCount }
      }));
      return lastResult;
    } finally {
      meta.setAttribute('content', original);
  ...
  function installRepairBench() {
    ...
    bench.innerHTML = `
      <strong>REPAIR BENCH r7 · STAGED WATCHDOG</strong>
      <div data-lab-repair-signature style="margin:6px 0"></div>
      <div data-lab-repair-gate style="margin:6px 0"></div>
  ...
  function confirmAutoRepair(reason) {
    if (repairInFlight || autoConfirmationTimer || !document.body || !autoAllowed()) return;
    ...
    autoConfirmationTimer = window.setTimeout(() => {
      ...
      if (repairInFlight || !document.body || !autoAllowed()) return;
