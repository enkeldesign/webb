(() => {
  ...
  document.documentElement.dataset.turnLab = 'dead-canyon-suburbs';
  ...
  function installGate() {
    ...
    installButton.addEventListener('click', () => {
      guideSteps.innerHTML = `
        ...
        <div class="install-step"><div class="install-step-number" aria-hidden="true">3</div><div><strong>Launch TURN LAB from its icon</strong><span>LAB keeps its own saves. DEAD CANYON uses the MOUNTAIN test slot and SUBURBS uses CLIFFSIDE.</span></div></div>`;
      guide.hidden = false;
    });
