...
const [index, releaseSource, postcardCss, chooserSource] = await Promise.all([
...
assert.match(
  index,
  new RegExp(`track-select-r77\\.css\\?build=${release.cacheKey}`),
  'Production must load the postcard layer through the current release cache key'
);
...
assert.match(postcardCss, /\.track-card-countryside \.track-card-preview[\s\S]*radial-gradient/, 'Countryside preview must read as fields and sun');
assert.match(postcardCss, /\.track-card-airport \.track-card-preview[\s\S]*linear-gradient/, 'Airport preview must read as sky and runway');
...
assert.doesNotMatch(postcardCss, /@keyframes|animation(?:-name)?:/, 'The visual refresh must add no looping or distracting motion');
...
console.log(`TURN ${release.id} distinct track palette and postcard previews passed.`);
