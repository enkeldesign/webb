...
assert.match(sharedHeadlight, /intensity: 2600/,
  'The shared spotlight should use the approved r174 racing-speed intensity');
assert.match(sharedHeadlight, /distance: 220/,
  'The shared spotlight should use the approved r174 220 m racing-speed range');
...
assert.match(sharedHeadlight, /lightLocal: Object\.freeze\(\{ x: 0, y: 0\.82, z: -0\.85 \}\)/,
  'The r174 emitter should sit close to the front of the car');
...
assert.match(mountainWrapper, /night-player-spotlight-r560\.js\?revision=r174-night-headlight-tune/,
  'MOUNTAIN must cache-bust to the r174 shared spotlight tune');
...
assert.match(midnight, /night-player-spotlight-r560\.js\?revision=r174-night-headlight-tune/,
  'MIDNIGHT CITY must cache-bust to the exact same r174 shared spotlight tune');
...
assert.match(registry, /mountain-world-r3\.js\?revision=r174-night-headlight-tune/,
  'Production must cache-bust MOUNTAIN to the r174 headlight tune');
assert.match(registry, /midnight-city-world-r11\.js\?build=20260818-r174-night-headlight-tune/,
  'Production must cache-bust MIDNIGHT CITY to the r174 headlight tune');

console.log('TURN shared MOUNTAIN + MIDNIGHT CITY r174 220 m shadowless spotlight contract passed.');
