...
} from '../audio/mayday-audio-r492.js?revision=r492';
...
const MAP_STYLE_ID = 'turn-airport-emergency-map-style-r492';
...
export function installAirportEmergency({ world, samples, runtime = globalThis.__turnRuntime } = {}) {
  ...
  function precompileEmergencyVisuals() {
    ...
    if (typeof renderer?.compileAsync !== 'function' || !camera) return Promise.resolve(false);
  ...
  function startResponderPrewarm() {
    if (runtime.state.vehicleId !== AMBULANCE_ID) return;
    // Do the GLTF/network work as soon as the Ambulance race is staged. The previous
    // requestIdleCallback could wait until the first lap was nearly over on iOS.
  ...
  function triggerCrash() {
    ...
    // Keep the finish-line event itself cheap: sound, state and instruction happen now;
    // scene reveal/reparenting happens just after the lap transition has rendered.
    playMaydayCrashSound();
  ...
  // Do not hang rescue logic from a crash-site mesh: once the wreck leaves the camera
  // frustum Three.js stops calling that mesh's onBeforeRender. Scene-level rendering
  // keeps the sirens and medical trigger alive all the way to the terminal.
  ...
  const onUiState = () => {
    if (!session.crashActive && runtime.state.vehicleId === AMBULANCE_ID) startResponderPrewarm();
    syncMapMarker();
    syncResponders();
  };
  ...
  world.userData.turnAirportEmergency = Object.freeze({
    ...
    audibleGuidance: 'positioned fire audio at the wreck and responder sirens at the terminal medical bay'
  });
...
function installMedicalSign(world, placement) {
  ...
  if (placement.terminal) {
    // Match the north-window centre exactly and cover the whole window rather than
    // looking like a small sign floating on top of it.
