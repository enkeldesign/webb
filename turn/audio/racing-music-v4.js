import { MENU_SONG, SONGBOOK, songForTrack } from './music/songbook.js?revision=r165-track-songbook-v1';
...
function stepDurationFor(song) {
  return (60 / song.bpm) / STEPS_PER_BEAT;
}
...
// Compatibility name retained for the picked chorus voice from the approved theme.
function playFluteLead(note, time) {
  ...
  body.frequency.exponentialRampToValueAtTime(hz, time + 0.025);
  harmonic.frequency.setValueAtTime(hz * 2.025, time);
  harmonic.frequency.exponentialRampToValueAtTime(hz * 2, time + 0.022);
  filter.type = 'lowpass';
  filter.Q.value = 1.2;
  filter.frequency.setValueAtTime(2000, time);
  filter.frequency.exponentialRampToValueAtTime(1000, endTime);
  amp.gain.setValueAtTime(0.0001, time);
  amp.gain.exponentialRampToValueAtTime(0.075, time + 0.008);
  amp.gain.exponentialRampToValueAtTime(0.025, time + duration * 0.38);
...
function playLead(note, time, { voice = 'lead' } = {}) {
  ...
  filter.Q.value = 0.45;
  scheduleGainEnvelope(amp.gain, time, 0.18, time + stepSeconds * 1.05, 0.022);
  ...
  body.start(time);
  overtone.start(time);
  body.stop(time + stepSeconds * 1.08);
  overtone.stop(time + stepSeconds * 1.08);
}
...
function playBass(note, time) {
  ...
  filter.frequency.exponentialRampToValueAtTime(280, time + stepSeconds * 0.95);
  scheduleGainEnvelope(amp.gain, time, 0.21, time + stepSeconds * 1.02, 0.016);
  ...
  body.start(time);
  sub.start(time);
  body.stop(time + stepSeconds * 1.05);
  sub.stop(time + stepSeconds * 1.05);
}
...
function playArp(note, time) {
  ...
  filter.Q.value = 0.35;
  scheduleGainEnvelope(amp.gain, time, 0.036, time + stepSeconds * 0.92, 0.012);
  oscillator.connect(filter);
  filter.connect(amp);
  amp.connect(masterGain);
  trackGraph(oscillator, [oscillator, filter, amp]);
  oscillator.start(time);
  oscillator.stop(time + stepSeconds * 0.95);
}
...
function activeSection() {
  return activeSong.arrangement[currentSection];
}

function scheduleStep(step, time) {
  const section = activeSection();
  playLead(section.lead[step], time, { voice: section.leadVoice || 'lead' });
...
function advanceStep() {
  ...
  if (currentStep >= section.lead.length) {
    currentStep = 0;
    currentSection = (currentSection + 1) % activeSong.arrangement.length;
  }
...
function switchSong(nextSong, { restart = true } = {}) {
  if (!nextSong || nextSong.id === activeSong.id) {
    if (restart && nextSong?.id === activeSong.id) resetSongPosition();
    return activeSong;
  }

  activeSong = nextSong;
  stepSeconds = stepDurationFor(activeSong);
  resetSongPosition();
  ...
  globalThis.dispatchEvent?.(new CustomEvent('turn:music-song-changed', {
    detail: { songId: activeSong.id, songName: activeSong.name, bpm: activeSong.bpm }
  }));
  return activeSong;
}

function resolveTrackId(detail = {}) {
...
function handleTrackChanged(event) {
  ...
  if (globalThis.__turnRuntime?.state?.running === true) {
    switchSong(songForTrack(selectedTrackId), { restart: true });
  }
}
...
function handleUiStateChange(event) {
  ...
  if (detail.running === true) {
    ...
    switchSong(songForTrack(trackId), { restart: false });
    return;
  }
...
export function installRacingMusic({ home = document.querySelector('.m8-home') } = {}) {
  ...
  selectedTrackId = globalThis.__turnGetTrackId?.() || globalThis.__turnRuntime?.state?.trackId || selectedTrackId;
  installStyles();
  ...
  globalThis.addEventListener('turn:track-changed', handleTrackChanged);
  globalThis.addEventListener('turn:ui-state-change', handleUiStateChange);
  ...
  const api = Object.freeze({
    timbre: 'warm-v4-track-songbook',
    songs: Object.freeze(SONGBOOK.map((song) => Object.freeze({
      id: song.id,
      name: song.name,
      bpm: song.bpm,
      form: song.form
    }))),
    get bpm() { return activeSong.bpm; },
    get songId() { return activeSong.id; },
    get songName() { return activeSong.name; },
    get arrangement() { return activeSong.form; },
