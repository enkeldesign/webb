...
const ARRANGEMENT = Object.freeze([TUNE, TUNE, BRIDGE]);
...
function applyMasterVolume() {
  ...
  const gain = soundEnabled && musicVolume > 0
    ? DESIGNED_MASTER_GAIN * (musicVolume / 100)
    : 0;
  try {
...
function installStyles() {
  ...
  style.textContent = `
    ...
    .turn-music-blank-toggle[hidden] {
      display: none;
    }
...
function installBlankScreenToggle() {
  ...
  if (blankControl) {
    observer.observe(blankControl, { attributes: true, attributeFilter: ['data-state', 'hidden', 'style'] });
  }
...
export function installRacingMusic({ home = document.querySelector('.m8-home') } = {}) {
  ...
  // Web Audio autoplay rules may keep the context suspended until the first user gesture.
  // The desired state is still ON by default; handleUserActivation starts it at the first
  // interaction that the browser permits to unlock audio.
  if (shouldPlay()) void startPlayback({ restart: false });
