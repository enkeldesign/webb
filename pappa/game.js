...
(() => {
  ...
  class Game {
    ...
    setup() {
      ...
      // Initially show scratch bubble at the stairs to hint about Bella.
    ...
    findPath(startX, startY, targetX, targetY) {
      ...
      if (this.grid[ty][tx] === 1) {
        // Search a small neighbourhood around the target for a passable cell
        let found = false;
        let nearest = null;
    ...
    update(dt) {
      ...
      if (!this.hpWarningShown && this.player.hp <= 25) {
        messageLog.add('Pappa gäspar…', 3000);
