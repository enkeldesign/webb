  ...
  class Game {
    ...
    setup() {
      ...
      this.canvas.addEventListener('click', e => {
        // Use offsetX/Y to get coordinates relative to the canvas, which
        // eliminates discrepancies caused by CSS scaling or margins.
        const px = e.offsetX - this.player.w / 2;
        const py = e.offsetY - this.player.h / 2;
        // Clamp into bounds so Pappa stays completely on the map
    ...
    buildLayout() {
       ...
       */
      // Horizontal walls at y=200.  Remove the middle segment near the hallway
      // so that the walkway fully opens into Arvids rum.  Only draw the
      // segments at the far left and far right.
