...
(() => {
  ...
  class Child extends Entity {
    ...
    draw(ctx) {
      ...
      // Determine which bubble to draw.  Highest priority: thank you
      // bubble; second: play request bubble during play quests; third:
      // normal call bubble when calling.
      // Highest priority: any explicit speech from speakLine
      if (this.speakTimer > 0 && this.speakLine) {
        drawBubble(this.speakLine);
      } else if (this.thankTimer > 0) {
  ...
  class Game {
    ...
    buildLayout() {
      ...
      // Upper part of wall
      obs.push({ x: doorWallX, y: 0, w: doorThickness, h: 80 });
      // Lower part of wall
      obs.push({ x: doorWallX, y: 140, w: doorThickness, h: 60 });
    ...
    findPath(startX, startY, targetX, targetY) {
      ...
      if (this.grid[ty][tx] === 1) {
        ...
        // Search radius of four cells in each direction
      ...
      switch (quest.type) {
        ...
        case 'snack': {
          ...
          if (quest.state === 'need_snack') {
            ...
            // Deliver snack when Pappa enters the child's room
            const room = child.room;
            if (this.player.carryItem === 'snack' && room && rectsIntersect(this.player.x, this.player.y, this.player.w, this.player.h, room.x, room.y, room.w, room.h)) {
              this.player.carryItem = null;
        ...
        case 'play': {
          ...
          if (quest.state === 'play_with_child') {
            if (child.name === 'Kerstin') {
              ...
              // Allow a larger radius when playing with Arvid so the
              // player doesn’t need to stand precisely on top of him.
              const playMargin = 40;
        ...
        case 'toapapper': {
          ...
          if (quest.state === 'need_paper') {
            ...
            // Deliver to the child's room; no need to collide directly with the child
            const room = child.room;
            if (this.player.carryItem === 'toapapper' && room && rectsIntersect(this.player.x, this.player.y, this.player.w, this.player.h, room.x, room.y, room.w, room.h)) {
              this.player.carryItem = null;
              messageLog.add(`${child.name}: Äntligen!`, 3000);
              // Move Kerstin back to sofa after receiving paper
              if (child.name === 'Kerstin') {
                child.x = 80;
                child.y = 240;
                // Ensure Kerstin stops any special walking state
